...
import {
  ...
  Shield, DollarSign, EyeOff, Eye, UserCog, Copy, KeyRound,
} from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  const { data: pwCodes } = useQuery({
    queryKey: ["admin-password-codes"],
    queryFn: () => get<PasswordCode[]>("/admin/password-codes"),
  ...
  return (
    <ErrorBoundary>
        <div className="space-y-fluid-4">
          ...
          {/* Password Change Codes (email fallback) */}
          <div className="glass p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                <KeyRound className="w-5 h-5 text-accent" /> Password Change Codes
              </h3>
              <span className="text-xs text-broker-400">Refreshes automatically</span>
            </div>
            <p className="text-xs text-broker-400 mb-4">
              Pending email-verification codes for password changes. When email delivery is unavailable, relay the code to the user manually.
            </p>
            {(!pwCodes || pwCodes.length === 0) ? (
              <div className="p-6 text-center text-broker-400 text-sm">No pending password change codes</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm" role="table">
                  ...
                  <tbody>
                    {pwCodes.map((c) => (
                      <tr key={c.id} className="border-b border-broker-700/30 hover:bg-surface-hover transition-colors">
                        <td className="px-4 py-3 text-broker-200">{c.email}</td>
                        <td className="px-4 py-3 font-mono text-white tracking-widest">{c.code}</td>
                        <td className="px-4 py-3 text-broker-300">{new Date(c.expiresAt).toLocaleTimeString()}</td>
                        <td className="px-4 py-3">
                          {c.emailed ? (
                            <span className="px-2 py-0.5 rounded text-xs font-medium bg-accent/10 text-accent">Emailed</span>
                          ) : (
                            <span className="px-2 py-0.5 rounded text-xs font-medium bg-amber-500/10 text-amber-400">Manual relay</span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-right">
                          <button
                            onClick={() => { navigator.clipboard?.writeText(c.code).catch(() => {}); toast.success("Code copied"); }}
